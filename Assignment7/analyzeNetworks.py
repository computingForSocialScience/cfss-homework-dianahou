...
def combineEdgeLists(edgeList1, edgeList2):
    '''Takes two dataframes as arguments and combines them into one edge list'''
...
def randomCentralNode(inputDiGraph):
    ...
    for key in ec_dict:
        ec_dict[key] = ec_dict[key]/float(sum(ec_dict.values()))  #each value from old dict is divided by sum of original values
    randomNode = np.random.choice(ec_dict.keys(), p=ec_dict.values())
    return randomNode
